...
// Motor de IA Geradora - Versão Ultra-Flash (Engine Proxy v4.3)
// Fixo: Faxina Rigorosa v4.3

/**
 * 🛠️ CHAMA O PROXY SERVERLESS (BACKEND)
 * Unifica a comunicação com a API da Google via Vercel Function /api/gemini
 */
async function callProxyAI({ prompt, fileBase64 = null, isOCR = false }) {
    ...
    if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Falha na comunicação com a IA (Serverless Proxy Error)');
    }
